...
class MeawowPredictorTest {

    /** Feed the same sample until the slew limiter settles, return the settled value. */
    private fun MeawowPredictor.settle(
        skin: Double,
        outer: Double,
        startMillis: Long = 0L,
        params: MeawowPredictor.Params = MeawowPredictor.Params.T201,
    ): Double {
        var t = startMillis
    ...
    fun `never displays below the skin temperature`() {
        val settled = MeawowPredictor().settle(31.4, 31.3)
    ...
    fun `t201_2 params weight the gradient about half as much`() {
        val t201 = MeawowPredictor().settle(35.0, 30.0)
        val t2012 = MeawowPredictor().settle(35.0, 30.0, params = MeawowPredictor.Params.T201_2)
